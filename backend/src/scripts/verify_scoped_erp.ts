...
async function main() {
  ...
  try {
    ...
    console.log("Login result user profile:", {
      ...
      managedDept: (loginResult.user as any).managedDept
    });

    if (!(loginResult.user as any).managedDept || (loginResult.user as any).managedDept.name !== 'ECE') {
    ...
    const hodFilteredPending = allPending.filter((u: any) => {
