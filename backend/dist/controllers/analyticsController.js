...
exports.getStudentAttendanceAnalytics = exports.getHODLeaveAnalytics = void 0;
const db_1 = require("../config/db");
/**
 * HOD Leave Analytics: Aggregates Approved vs Rejected leaves per month
 */
const getHODLeaveAnalytics = async (req, res) => {
    try {
        ...
        leaves.forEach(leave => {
            ...
            if (aggregatedMap[monthName]) {
                if (leave.status === 'APPROVED')
                    aggregatedMap[monthName].approved++;
                if (leave.status === 'REJECTED')
                    aggregatedMap[monthName].rejected++;
            }
        });
...
exports.getHODLeaveAnalytics = getHODLeaveAnalytics;
/**
 * Student Attendance Analytics: Calculates percentage and returns pie chart data
 */
const getStudentAttendanceAnalytics = async (req, res) => {
    try {
        const userId = req.userId;
        ...
        const stats = await db_1.prisma.attendance.groupBy({
            by: ['status'],
...
exports.getStudentAttendanceAnalytics = getStudentAttendanceAnalytics;
